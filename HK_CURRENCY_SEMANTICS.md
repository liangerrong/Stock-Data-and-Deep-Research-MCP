...
- 东方财富港股金额：当报告列表、主要指标和实际金额行为无法证明使用同一币种时，标为 `UNRESOLVED`，只作核对，不进入模型。
...
1. 报告列表的 `CURRENCY` 只保存为“发行人报告币种标签”。
2. 东方财富行项目金额统一放入 `provider_statements_long.csv`。
3. 其 `currency` 固定为 `UNRESOLVED`，同时保留 `issuer_reported_currency` 和 `provider_claimed_currency`。
4. 这些金额不得自动并入 `core_actuals.csv`，也不得用于估值。

## 当前机械化边界

- Yahoo Finance 通常可提供五个年度列，但个别公司季度表可能为空或不足。
- 东方财富分红方案文字通常明确声明港币、美元等币种，可逐条解析，不继承财报币种。
- 港股产品/地区分部与报告级 HKEX 官方链接当前没有可靠的现成 AKShare 接口，保持空表并输出 `WARN`。
