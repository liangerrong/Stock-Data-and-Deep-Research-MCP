...
async def handle_list_tools() -> list[types.Tool]:
    return [
        types.Tool(
            name="get_financials",
            description="Fetch the current market snapshot and recent years of financial data, save them to the current directory, and return the local file paths.",
            inputSchema={
                "type": "object",
                "properties": {
                    "stock_code": {
                    ...
                    "years": {
                        "type": "integer",
                        "description": "Number of recent years of financial data to fetch (default: 1)"
                    }
                },
                "required": ["stock_code"]
            }
...
async def handle_call_tool(
    ...
    if name == "get_financials":
        ...
        years = arguments.get("years", 1)
        
        if not stock_code:
            raise ValueError("stock_code string argument is required")
            
        result = handle_get_financials(stock_code, years)
        ...
        result = handle_search_stock(stock_name)
        return [types.TextContent(type="text", text=result)]
